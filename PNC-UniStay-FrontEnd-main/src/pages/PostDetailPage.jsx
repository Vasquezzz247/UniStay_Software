...
import {
  ...
  FaDollarSign, FaTag, FaUserCircle, FaRegCommentDots, FaCalendarCheck,
  FaFileSignature, FaWifi, FaBookmark
} from 'react-icons/fa';
import {
...
function PostDetailPage() {
  ...
  // ✅ comprobar si este post ya está guardado
  useEffect(() => {
    const checkBookmark = async () => {
      try {
        const token = localStorage.getItem('userToken');
        if (!token) return; // si no está logueado, no hacemos nada
  ...
  if (post.roomDetails) {
    if (post.roomDetails.squareFootage)
      roomFeatures.push({ icon: <MdSquareFoot size="1.8em" className="mb-1 text-sky-600" />, title: `${post.roomDetails.squareFootage} m²`, detail: "Tamaño" });
    if (post.roomDetails.bathroomType)
      roomFeatures.push({ icon: <FaBath size="1.8em" className="mb-1 text-sky-600" />, title: post.roomDetails.bathroomType, detail: "Baño" });
    if (post.roomDetails.kitchenType)
      roomFeatures.push({ icon: <FaUtensils size="1.8em" className="mb-1 text-sky-600" />, title: post.roomDetails.kitchenType, detail: "Cocina" });
    if (post.roomDetails.isFurnished !== undefined)
      roomFeatures.push({ icon: <FaChair size="1.8em" className="mb-1 text-sky-600" />, title: post.roomDetails.isFurnished ? "Sí" : "No", detail: "Amoblado" });
  ...
  const amenitiesList = (post.amenities && post.amenities.length > 0)
    ? post.amenities.map(amenityText => ({ text: amenityText.text || amenityText }))
    : [];
      ...
      <div className="lg:grid lg:grid-cols-3 lg:gap-8 xl:gap-12">
        ...
        <aside className="lg:col-span-1 space-y-6 mt-10 lg:mt-0">
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 space-y-3 sticky top-24">
            {/* Botón de guardar / quitar de guardados - ambos roles */}
            {userRole && (
              <button
            ...
            {/* Acción rol propietario opcional */}
